...
### 3-2. 산출결과(스냅샷) — 2026-09-05 결정: 저장 안 함, 매 요청 계산

Y~AH는 원래 별도 컬렉션(`recommendationSnapshots`)에 저장하는 안을 검토했으나(PK = 유저ID+장소ID, 최신값만 유지) **보류/생략으로 결정**. 근거:

1. **로그인·세션ID 없음** — 저장하려면 디바이스/세션 식별자를 새로 만들어 서버로 보내야 하는데, 이건 "로그인 없음 · Play Store 수집 데이터 없음"이라는 아키텍처 원칙(CLAUDE.md)과 정면 충돌한다.
2. **설명가능성 요구는 이미 충족됨** — "왜 이 추천인지" 요구는 매 응답의 `추천이유` 텍스트로 실시간 제공되므로, 저장 없이도 목적 달성.
3. **행동 데이터 학습은 제출 이후 로드맵**(CLAUDE.md 발전계획) — 지금 스프린트 범위 아님.
4. MongoDB M0 무료티어(512MB) 절약.

**부활 조건**: 로그인/세션 개념이 생기거나(예: 회원제 전환), 제출 후 "행동 데이터 학습" 로드맵에 실제 착수할 때 재검토. 그 전까지 `/api/recommend`는 요청마다 Y~AH를 계산해서 응답에만 포함하고 저장하지 않는다.
...
| ~~`recommendationSnapshots`~~ | **생략(3-2 참고)** — Y~AH는 저장 없이 요청마다 계산 |
