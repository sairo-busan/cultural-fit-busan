...
- **현재 동작**: `/api/recommend`는 요청마다 서버리스 함수가 기동되어 MongoDB를 재조회한다. 한동안 요청이 없다가 처음 부르면(콜드스타트) 1.15초, 이후엔 0.1초로 측정됨(소피 실측).
...
소피 제안 원문 요지: 지금은 `cache-control: max-age=0`이라 매번 함수로 감. CDN에 60초 캐시하면 요금제 변경·재빌드 없이 같이 빨라짐. 단, DB를 고친 직후엔 다음 방문자 한 명까지 최대 60~360초(s-maxage 60 + stale-while-revalidate 300) 이전 값이 보일 수 있음(그 요청이 뒤에서 새 값을 받아옴).
...
소피 제안 코드 그대로 반영. 사진·TTS 작업으로 DB를 자주 고치는 중이라 stale 창(최대 1명, 최대 360초)이 실제로 열려있는 상태에서 적용하기로 사용자 결정.

### TODO — 제출 전 stale-while-revalidate 늘리기 (소피 제안, 9/17)

데이터 작업(사진·TTS·영문 필드) 다 끝나고 제출 직전에
`stale-while-revalidate=300` → `86400`으로 늘릴 것. 오랜만에 온 사람도
캐시된 값을 바로 받고 새 값은 뒤에서 받아오게 됨. 배포하면 캐시는 비워지므로
그 시점부터 안전하게 적용 가능 — `src/app/api/recommend/route.ts` 한 줄.
