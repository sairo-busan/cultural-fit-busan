...
### 저장 탭 100건 상한 (서버는 고쳤음, 소피 쪽 후속 필요)

`/api/recommend`의 `limit` 상한을 이 PR에서 100→120으로 올렸다(DB_01이
120건이라). 근데 소피 PR#18의 저장 탭(`SavedContent.tsx`)이 `?limit=100`을
코드에 직접 박아놨다 — 서버 상한만 올려서는 그 호출 자체가 여전히 100건까지만
받는다. #18 리뷰에서 이미 안내함, 머지 후 그쪽 호출도 120으로 맞추거나 id
직접조회 API로 바꿔야 한다.
