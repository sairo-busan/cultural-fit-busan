...
| 2 | `/privacy` 버튼 | **확정** `AppHeader onBack` — S20 실패 화면 · S03 과 같은 헤더 |
| 3 | 기록이 없을 때 | **확정** S20 → `/feed` (`Hero` 와 같음) · `/privacy` → `/me` (방침 링크가 있는 탭) |
| 4 | `/privacy` 서버 컴포넌트 | **확정** 본문은 서버 그대로 두고, 뒤로 버튼만 작은 클라이언트 컴포넌트로 얹는다 (`AppHeader` 는 함수 prop 을 받아 서버에서 바로 못 쓴다) |
...
- [x] S20 불러오기 실패 화면에 뒤로 버튼이 보이고, 누르면 이전 화면으로 간다
- [x] S20 찾을 수 없음 화면에 뒤로 버튼이 보인다
- [x] `/privacy` 에 뒤로 버튼이 보이고, 내 정보 탭에서 들어왔으면 내 정보 탭으로 돌아간다
- [x] 주소를 바로 열어 기록이 없을 때 S20 은 `/feed`, `/privacy` 는 `/me` 로 간다
- [x] 뒤로 버튼에 화면 낭독기 이름(`nav.back`)이 있고 터치 영역이 44px 이상이다
- [ ] 앱(Capacitor)에서 상태바와 겹치지 않는다 (`pt-safe-header`) — 017 · 003 머지 후 앱 빌드에서 함께 확인
- [x] `tsc --noEmit` · `lint` · `build` 통과 (`lint` 는 `scripts/` 기존 에러 4건 외 없음)
...
| 파일 | 내용 |
|---|---|
| `place/[id]/PlaceContent.tsx` | 뒤로 가기 규칙(`back`)을 `Hero` 에서 본문으로 올려 사진 위 버튼 · 실패 화면이 같이 쓴다. `failed` · `notFound` 분기에 `AppHeader onBack` |
| `privacy/BackHeader.tsx` | 새 파일. `AppHeader onBack` 을 감싼 클라이언트 컴포넌트, 기록 없으면 `/me` |
| `privacy/page.tsx` | 헤더를 본문 위에 두고 본문 위쪽 여백 `py-10` → `pt-2` |

헤더는 `.screen` 밖에 둔다. 안에 두면 좌우 여백이 두 번 들어가 버튼이 x=34 로 밀린다 (다른 헤더 화면은 x=14).

### 확인 (로컬 개발 서버 · 392×812)

| 상황 | 결과 |
|---|---|
| 정상 상세 | 사진 위 흰 원 버튼 그대로 |
| 불러오기 실패 ko · en · 찾을 수 없음 | 헤더 뒤로 버튼 x=14 |
| 추천 → 상세(실패) → 뒤로 | `/ko/feed` |
| 기록 없는 새 탭 — 찾을 수 없음 · 불러오기 실패 → 뒤로 | `/ko/feed` |
| 내 정보 → 방침 → 뒤로 | `/ko/me` |
| 기록 없는 새 탭 — 방침 → 뒤로 | `/ko/me` |

전후 캡처: `docs/screens/FE-BUG-003/`

### 남은 것

- 앱 상태바 — 017 · 003 이 main 에 들어간 뒤 앱 빌드 한 번으로 016 · 004 · 017 과 함께 확인
- 뒤로 가기 규칙은 "탭 기록이 1개보다 많은가" 만 본다. 다른 사이트에서 링크로 들어오면 그 사이트로 돌아간다 (브라우저 뒤로 가기와 같음, 앱에선 해당 없음). 같은 코드가 다섯 곳(S03 · 취향 진단 · 취향 결과 · S20 · 방침)이라 모으는 정리와 함께 별건
